...
export default function Dashboard() {
  ...
  // Resetar progresso do desafio se for nova semana
  if (userProgress && activeChallenge) {
    ...
    const lastUpdated = userProgress.updated_date ? new Date(userProgress.updated_date) : null;
    
    // Se última atualização foi antes desta segunda, resetar progresso
  ...
  const handleAddProgress = () => {
    ...
    // Verificar se precisa resetar o progresso (nova semana)
    const today = new Date();
    const currentDay = today.getDay();
    const daysSinceMonday = currentDay === 0 ? 6 : currentDay - 1;
    const thisMonday = new Date(today);
    ...
    // Se o último update foi antes desta segunda-feira, resetar
    if (lastUpdated && lastUpdated < thisMonday) {
      currentProgress = 0;
    }
  ...
  // Calcular treinos da semana (segunda a domingo)
  const thisWeekWorkouts = workoutLogs.filter(log => {
    ...
    const daysSinceMonday = currentDay === 0 ? 6 : currentDay - 1; // Se domingo, é 6 dias depois da segunda
    const thisMonday = new Date(today);
    thisMonday.setDate(today.getDate() - daysSinceMonday);
    thisMonday.setHours(0, 0, 0, 0);
    ...
    return logDate >= thisMonday && logDate <= thisSunday;
  });
